...
sale.click('quote')
assertEqual(sale.untaxed_amount, sale.tax_amount, sale.total_amount
...
assertEqual(sale.state
, 'processing'
)
assertEqual(sale.shipment_state
, 'waiting'
)
assertEqual(sale.invoice_state
, 'pending'
)
assertEqual(len(sale.shipments), len(sale.shipment_returns), len(sale.invoices)
, (1, 0, 1)
)
...
stock_move1, stock_move2 = sorted(shipment.outgoing_moves,
    key=lambda m: m.quantity or 0)
assertEqual(len(stock_move1.invoice_lines)
, 0
)
assertEqual(len(stock_move2.invoice_lines)
, 0
)
# 
Validate Shipments::
...
sale.reload()
assertEqual(len(sale.invoices)
, 2
)
assertEqual(sum(l.quantity for i in sale.invoices for l in i.lines)
, 5.0
)
# 
...
assertEqual(len(invoice_line.stock_moves)
, 0
)
# 
Post and Pay Invoice for 4 products::
invoice_line, = invoice.lines
assertEqual(invoice_line.quantity
, 5.0
)
...
assertEqual(stock_move.quantity
, 4.0
)
...
assertEqual(stock_inventory_move.quantity
, 4.0
)
...
shipment.click('pack')
shipment.click('do')
assertEqual(shipment.state
, 'done'
)
# 
New shipments created::
sale.reload()
assertEqual(len(sale.shipments)
, 2
)
# 
...
assertEqual(return_.state
, 'processing'
)
assertEqual(return_.shipment_state
, 'waiting'
)
assertEqual(return_.invoice_state
, 'none'
)
assertEqual((len(return_.shipments), len(return_.shipment_returns),
...
assertEqual(move_return.product.rec_name
, 'product'
)
assertEqual(move_return.quantity
, 4.0
)
...
assertEqual(credit_note.type
, 'out'
)
assertEqual(len(credit_note.lines)
, 1
)
assertEqual(sum(l.quantity for l in credit_note.lines)
, -3.0
)
...
mix.click('quote')
mix.click('confirm')
assertEqual(mix.state
, 'processing'
)
assertEqual(mix.shipment_state
, 'waiting'
)
assertEqual(mix.invoice_state
, 'pending'
)
assertEqual(len(mix.shipments), len(mix.shipment_returns), len(mix.invoices)
, (1, 1, 1)
)
# 
Checking Shipments::
...
assertEqual(move_shipment.product.rec_name
, 'product'
)
assertEqual(move_shipment.quantity
, 7.0
)
# 
...
assertEqual(mix_invoice.type
, 'out'
)
assertEqual(len(mix_invoice.lines)
, 2
)
assertEqual(sorted(l.quantity for l in mix_invoice.lines)
, [-2.0, 7.0]
)
...
assertEqual(service_sale.state
, 'processing'
)
assertEqual(service_sale.shipment_state
, 'none'
)
assertEqual(service_sale.invoice_state
, 'pending'
)
...
assertEqual(service_invoice.state
, 'paid'
)
# 
...
assertEqual(sale_to_return.state
, 'processing'
)
...
assertEqual(sorted([x.quantity or 0 for x in returned_sale.lines])
, [-1.0, 0]
)
# 
...
new_invoice.number
assertEqual(len(new_invoice.lines)
, 1
)